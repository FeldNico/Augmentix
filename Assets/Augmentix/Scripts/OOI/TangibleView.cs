...
namespace Augmentix.Scripts.OOI
{
    public class TangibleView : MonoBehaviourPunCallbacks, IPunObservable
    {
        ...
        public void Update()
        {
            ...
            {
                transform.localPosition = Vector3.MoveTowards(transform.localPosition, this.m_NetworkPosition,
                    this.m_Distance * (1.0f / PhotonNetwork.SerializationRate));
                transform.localRotation = Quaternion.RotateTowards(transform.localRotation, this.m_NetworkRotation,
